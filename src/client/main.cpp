...
int main(int argc,char**argv)
{
    if(argc<3)
    {
        cout<<"command invalid! example: ./ChatClient 127.0.0.1 1221"<<endl;
        exit(-1);
    }
    //读取ip和端口
    char* ip=argv[1];
    ...
    for(;;)
    {
        ...
        case 1: // 处理登录业务
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (responsejs.contains("msg"))
                        {
                            vector<string> vec = responsejs["msg"];
                            for (string &str : vec)
                            {
                                json js = json::parse(str);
                        ...
                        //在loginout时只是退出聊天界面，程序并没有结束，所以当再次登录时，又会重新开辟一个线程
                        //所哟用一个标识符
            ...
            case 2:
            {
                ...
                //然后等待信号量
                //sem_wait(&rwsem);//当子线程处理完注册任务后，会通知返回
                else
                {
                    char buf[1024] = {0};
                    // 接收来自服务器转发的消息
